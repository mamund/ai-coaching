# Tool Shed Sharing Program (Vocabulary-Annotated)
...
These properties define the shared vocabulary of the system and are reused across all resources and actions. Each property now includes a `vocabRef` annotation linking to canonical vocabulary terms.
...
* **toolType**
  ...
  * **Type:** string
  * **Example:** `"Drill"`
  * **vocabRef:** `schema:additionalType`
...
* **isOverdue**
  ...
  * **Type:** boolean
  * **Example:** `false`
  * **vocabRef:** `ex:isOverdue` *(Note: No exact Schema.org match; custom extension recommended)*
...
* **depositRequired**
  ...
  * **Type:** boolean
  * **Example:** `true`
  * **vocabRef:** `ex:depositRequired` *(Note: No exact Schema.org match; custom extension recommended)*
...
This document has been annotated with canonical vocabulary references from Schema.org. Two properties (`isOverdue` and `depositRequired`) have been marked for custom extension in the `ex:toolshed` namespace, as they represent domain-specific concepts without direct Schema.org equivalents.

For machine processing, the namespace context block at the top of this document provides the prefix mappings needed to resolve all vocabulary references.

---

**Validation Summary:** PASS  
This annotated API Story conforms to the Vocabulary Coach standard and includes complete vocabulary alignment.
