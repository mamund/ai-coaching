...
Version: v0.2
Status: Rules locked from recent sessions, includes ROLES support
...
The story is the semantic authority. The processor must not invent affordances, inputs, routes, query params, requiredness, or authorization rules.
...
- Optional AllowedRoles elements on resources and actions (only when ROLES exists)
...
- Every property has a resolvable type hint.
- Every rule is either recognized and enforced, or explicitly ignored with a warning.
- If ROLES is present, every resource and every action declares explicit AllowedRoles with no inheritance.
...
## 3.5 ROLES and AllowedRoles rules

### 3.5.1 Consistency lock

- If ROLES is defined, AllowedRoles MUST be present on every resource and every action.
- If ROLES is NOT defined, AllowedRoles MUST NOT appear anywhere in the story.

This eliminates inheritance, precedence ambiguity, and partial security models.

### 3.5.2 Role vocabulary validation

When ROLES is defined:

- Every role token referenced in any AllowedRoles list MUST exist in ROLES.
- AllowedRoles lists MUST contain no duplicates.
- ROLES itself MUST contain no duplicates.
...
- Preserve role tokens exactly as declared.
...
- resources: name, plural name, description, allowedRoles (optional)
...
- actions: name, resource association, inputs, requiredInputs, allowedRoles (optional), method, scope (home, collection, item), routeTemplate, inputLocations
...
- [ ] Enforce ROLES and AllowedRoles consistency lock.
